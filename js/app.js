...
function generateText(e) {
  e.preventDefault()
  let inputNum = amountText.value
  let randomNum = Math.floor(Math.random() * dummyText.length)

  if(inputNum < 0 || inputNum > 9 || inputNum === "" || isNaN(inputNum)) {
    result.innerHTML = dummyText[randomNum]
  } else {
    let slicedArry = dummyText.slice(0,inputNum)
    let finalArry = slicedArry.map((item) => {
      return `<p>${item}</p>`
    }).join('')
